...
namespace ev.test.persistence.events
{
    public class ArriveEventDataTests
    {
        ...
        {
            var configuration = new MapperConfiguration(cfg => cfg.AddMaps(typeof(ArriveEventData).Assembly));
            var mapper = new Mapper(configuration);

            var arriveEvent = new ArriveEvent(DateTime.Now, new Ship("ship id", "", "", Port.AT_SEA), new Port() { Id = "port id", Name = "port name", IntlCode = "intl code" });
